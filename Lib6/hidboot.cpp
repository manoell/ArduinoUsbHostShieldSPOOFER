...
void MouseReportParser::Parse(USBHID *hid __attribute__((unused)), bool is_rpt_id __attribute__((unused)), uint8_t len __attribute__((unused)), uint8_t *buf) {
        ...
        if (prevState.mouseInfo.bmBuckButton == 0 && pmi->bmBuckButton == 1)
                OnBackButtonDown(pmi);

        if (prevState.mouseInfo.bmBuckButton == 1 && pmi->bmBuckButton == 0)
                OnBackButtonUp(pmi);

        if (prevState.mouseInfo.bmForwardButton == 0 && pmi->bmForwardButton == 1)
                OnForwardButtonDown(pmi);

        if (prevState.mouseInfo.bmForwardButton == 1 && pmi->bmForwardButton == 0)
        ...
        if (prevState.mouseInfo.bmBackButton != pmi->bmBackButton) {
            if (pmi->bmBackButton) {
                OnBackButtonDown(pmi);
            }
            else {
        ...
        if (prevState.mouseInfo.bmForwardButton != pmi->bmForwardButton) {
            if (pmi->bmForwardButton) {
                OnForwardButtonDown(pmi);
            }
            else {
                OnForwardButtonUp(pmi);
